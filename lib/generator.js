...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		i++;
		file.path = path.join(os.tmpdir(), 'spritesheet_js_' + uuid + "_" + (new Date()).getTime() + '_image_' + i + '_test.png');
		...
		exec('magick convert' + scale + ' ' + fuzz + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim "' + file.path + '"', next);
	}, callback);
};
...
exports.getImagesSizes = function (files, options, callback) {
	...
	exec('magick identify ' + filePaths.join(' '), function (err, stdout) {
		console.log(err)
...
function makeCommands(fileList, options, index) {
	return [
		`magick convert -define png:exclude-chunks=date -quality 0% -size ${options.width}x${options.height} xc:none`,
		index !== 0 &&
